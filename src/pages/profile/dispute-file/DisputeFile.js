...
import {disputeFile, fetchContractDetails} from 'src/services/http.service';
...
class DisputeFile extends Component {
  ...
  onDispute = () => {
    ...
    if (freelancerInput && selfInput && milestoneId?.length) {
      ...
      if (!milestoneId?.length) {
        Snackbar.show({
          text: 'Please select milestone or full amount of project',
  ...
  componentDidMount() {
    ...
    fetchContractDetails(this.props.token, params?.contractId)
      .then((res) => {
  ...
  render() {
    ...
    return (
      <>
        <Header
          backButton={true}
          title={t('fileDispute.refundRequested')}
          navigation={this.props.navigation}
          notificationButton={true}
        />
        <ScrollView style={styles.container}>
          ...
          <TouchableOpacity
            ...
            <Text
              ...
              {t('fileDispute.fullAmount')}
            </Text>
          </TouchableOpacity>
          <Text style={styles.label}>{t('fileDispute.refundRequested')}</Text>
          <View style={{flexDirection: 'row'}}>
          ...
          <Text style={styles.label}>{t('fileDispute.noteFreelancer')}</Text>
          <TextInput
            numberOfLines={4}
          ...
          <Text style={styles.label}>{t('fileDispute.noteSelf')}</Text>
          <TextInput
            numberOfLines={4}
            style={styles.describeInput}
            placeholder={t('fileDispute.describeHere')}
          ...
          <CustomButton
            title={t('fileDispute.fileADispute')}
