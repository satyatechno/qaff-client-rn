...
export default function PaymentDetails({navigation, route}) {
  ...
  return (
    <View style={{backgroundColor: colors.defaultWhite, flex: 1}}>
      <Header
        backButton
        title={i18n.t('profilePaymentDetails.paymentDetails')}
        notificationButton
        navigation={navigation}
      />
      <View style={styles.cardTypeContainer}>
        <Text style={styles.cardTypeText}>
          {i18n.t('profilePaymentDetails.cardType')}
        </Text>
      ...
      <Text style={styles.cardNumberText}>
        {i18n.t('profilePaymentDetails.name')}
      </Text>
      ...
      {nameError && (
        <View style={{marginStart: -10, marginTop: -10, marginBottom: 10}}>
          <ErrorText name={i18n.t('profilePaymentDetails.nameRequired')} />
        </View>
      )}
      <Text style={styles.cardNumberText}>
        {i18n.t('profilePaymentDetails.cardNumber')}
      </Text>
      ...
      {cardNumberError && (
        <View style={{marginStart: -10, marginTop: -10, marginBottom: 10}}>
          <ErrorText name={i18n.t('profilePaymentDetails.cardNoRequired')} />
        </View>
      )}
      <View style={styles.expAndCvvContainer}>
        <View style={styles.expContainer}>
          <Text style={styles.expText}>
            {i18n.t('profilePaymentDetails.expDate')}
          </Text>
          <TouchableOpacity
            onPress={() => {
              showPicker(true);
          ...
          {dateError && (
            <View style={{marginStart: -20}}>
              <ErrorText name={i18n.t('profilePaymentDetails.dateRequired')} />
            </View>
          )}
        </View>
        <View style={styles.cvvContainer}>
          <Text style={styles.cvvText}>
            {i18n.t('profilePaymentDetails.cvv')}
          </Text>
          <TextInput
            returnKeyType="done"
            value={cvv}
          ...
          {cvvError && (
            <View style={{marginStart: -20}}>
              <ErrorText name={i18n.t('profilePaymentDetails.cvvRequired')} />
            </View>
          )}
        </View>
      </View>
      ...
      <TouchableOpacity onPress={checkValidation} style={styles.saveCardButton}>
        {loading ? (
          ...
          <Text style={styles.saveCardText}>
            {i18n.t('profilePaymentDetails.saveCard')}
          </Text>
        )}
      </TouchableOpacity>
      {showDate && (
