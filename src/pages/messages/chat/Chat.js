...
class Chat extends Component {
  ...
  componentDidMount() {
    ...
    if (chatDialogs?.length) {
      console.log('a', chatDialogs?.length);
      for (let i = 0; i < chatDialogs.length; i++) {
        ...
        if (chatDialogs[i].occupantsIds.find((id) => id === quickbloxId)) {
          ...
          console.log('dialog already exists', chatDialogs[i]?.id);
          QB.chat
            .getDialogMessages({
      ...
      QB.chat
        ...
        .then((dialog) => {
          ...
          QB.chat
            ...
            .then((result) => {
              console.log('dialog messages', result?.messages[0]);
    ...
    if (session?.token.length && isChatConnected) {
      if (message?.attachments?.length || message?.body !== null) {
        QB.chat
          ...
          .catch((err) => {
            console.log('Couldnot send message', err);
    ...
    if (index == 0) {
      ...
      try {
        ...
        const results = await DocumentPicker.pickMultiple({
          type: [pdf, zip, csv, plainText, '.mp4', 'video/*'],
        });
