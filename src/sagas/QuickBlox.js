...
function* quickbloxSetup() {
  ...
  console.log('qb id pass', login, password);
  yield put({
  ...
  try {
    ...
    while (appStateChannel) {
      try {
        ...
        console.error('appstate channel error:', err);
      }
    }
  } catch (err) {
    console.log('Couldnot setup app state channel', err);
  }
}
